...
export default function App() {
  ...
  useEffect(()=>{
    const url = process.env.REACT_APP_API_ROUTE + "/pages";
    fetch(url)
      .then((res) => {
        if (res.ok) {
          return res.json();
  ...
  useEffect(() => {
    ...
    if (visiblePageIndexs.length === 0) {
      ...
      const page = pages.find(
        (x:any) => x.index === Math.max(...visiblePageIndexs)
      );
            ...
            <Grid item xs zeroMinWidth>
              ...
              >
                <Routes>
                  ...
                  />
                  {pages.map(({ index, content, route }) => (
                    <Route
                      key={index}
                      path={route}
                      element={<MDContainer content={content} />}
                    />
                  ))}
